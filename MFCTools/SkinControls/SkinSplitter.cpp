...
BOOL CSkinSplitter::OnEraseBkgnd(CDC * pDC)
{
	if(m_pszResource)
	{
		...
		//渲染资源
		CSkinRenderManager * pSkinRenderManager=CSkinRenderManager::GetInstance();
		if(pSkinRenderManager)
		{
			pSkinRenderManager->RenderImage(ImageBack);
		}
		ImageBack.DrawImageTile(pDC,rcClient);
		/*
		DWORD dwWidth = ImageBack.GetWidth();
		DWORD dwHeight= ImageBack.GetHeight();
		DWORD dwBottom= rcClient.bottom;
		
		for(int nYPos = 0;nYPos < (int)dwBottom;nYPos+=(int)dwHeight)
		{
			for (INT nXPos=0;nXPos<rcClient.Width();nXPos+=dwWidth)
			{
				ImageBack.BitBlt(pDC->m_hDC,nXPos,nYPos,__min(rcClient.right-nXPos,dwWidth),__min(dwBottom-nYPos,dwHeight),0,0);
			}
		}*/
	}
	else
...
VOID CSkinSplitter::SetBackGround( UINT uID,HINSTANCE hResInstance )
{
	m_pszResource = MAKEINTRESOURCE(uID);
