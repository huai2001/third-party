...
//菜单属性
class SKIN_CONTROLS_CLASS CSkinMenuAttribute
{
	//字体颜色
public:
...
//菜单内核
class SKIN_CONTROLS_CLASS CSkinMenuKernel : public CMenu
{
	//菜单属性
public:
	...
	//HOOK 进程
	static HHOOK m_Hook;
	static volatile long m_HookRefcount;

protected:
	CBitImage m_ImageMenuFlags;
	//函数定义
public:
	//构造函数
	CSkinMenuKernel();
...
//数组说明
typedef CArray<CSkinMenuItem *>			CMenuItemArray;					//子项数组
typedef CArray<CSkinMenuImage *>		CMenuImageArray;				//子项数组
typedef CArray<CSkinMenuString *>		CMenuStringArray;				//子项数组
typedef CArray<CSkinMenuSeparator *>	CMenuSeparatorArray;			//子项数组

//界面菜单
class SKIN_CONTROLS_CLASS CSkinMenu
{
	//内核变量
protected:
