...
bool CPNGImage::LoadImage( const BYTE* pStart, int nMemSize )
{
	if (!pStart || (nMemSize <= 0))
		return false ;

	bool      bRet = false ;
	HGLOBAL   hGlobal = ::GlobalAlloc (GMEM_MOVEABLE, nMemSize) ;

	void* pData = GlobalLock(hGlobal);
	memcpy(pData,pStart,nMemSize);
	::GlobalUnlock (hGlobal) ;

	IStream     * pIStream = NULL ;
	if (::CreateStreamOnHGlobal (hGlobal, TRUE, &pIStream) != S_OK) return false;

	//写入数据
	pIStream->Write(pData,nMemSize,NULL);
