...
//公共定义
	...
	//圆角大小
#define ROUND_CX					2									//圆角宽度
#define ROUND_CY					2									//圆角高度

	//阴影定义
#define SHADOW_CX					2									//阴影宽度
#define SHADOW_CY					2									//阴影高度
...
{
	//标题颜色
	m_crTitleText = RGB(255,255,255);
	m_crTitleFrame = RGB(0,96,159);

	//控件颜色
	m_crControlText = RGB(10,10,10);
	m_crBackGround = RGB(229,249,255);
...
VOID CSkinDialog::CaleTitleButton(INT nWidth, INT nHeight)
{
	...
	for (INT i=0;i<CountArray(m_rcButton);i++)
	{
		//计算偏移
		INT nYPos=3;
		INT nXPos=nWidth-(m_SkinAttribute.m_SizeButton.cx+2)*(i+1) - 5;
...
VOID CSkinDialog::DrawSystemTitle(CDC * pDC, INT nWdith, INT nHeight)
{
	...
	if (m_hIcon == NULL) {
		m_hIcon = GetIcon(FALSE);
		//加载图标
		CPNGImage ImageLogo;
		ImageLogo.LoadImage(GetModuleHandle(SKIN_CONTROLS_DLL_NAME),TEXT("SKIN_WND_LOGO"));

		//绘画图标
		if (ImageLogo.IsNull()==false)
		{
			ImageLogo.DrawImage(pDC, 5, 6, 16, 16, 0, 0, ImageLogo.GetWidth(), ImageLogo.GetHeight());
		}
	} else {
		DrawIconEx(pDC->GetSafeHdc(), 5, 6, m_hIcon, 16, 16, 0, NULL, DI_NORMAL | DI_COMPAT);
	}
	...
	if (strTitle.IsEmpty()==false)
	{
		...
		//计算区域
		CRect rcTitle;
		rcTitle.SetRect(25,2,nWdith - 95, m_SkinAttribute.m_EncircleInfoFrame.nTBorder);
...
//取消按钮
VOID CSkinDialog::OnCancel()
{
	//隐藏分层
	if (m_SkinLayered.m_hWnd != NULL)
	{
		m_SkinLayered.ShowWindow(SW_HIDE);
	}
...
VOID CALLBACK CSkinDialog::SetWndFont(CWnd *pWnd, CFont *pFont)
{
	...
	if(pSetFont == NULL) {
		pSetFont = &CSkinResourceManager::GetInstance()->GetDefaultFont();
	}
