...
VOID CSkinButton::RectifyControl()
{
	...
	else
	{
		//加载资源
		CBitImage ImageButton;
		ImageButton.LoadFromResource(GetModuleHandle(SKIN_CONTROLS_DLL_NAME),IDB_SKIN_BUTTON);
...
VOID CSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	bool bButtonDown=((lpDrawItemStruct->itemState&ODS_SELECTED)!=0 /*||(m_nCheck != 0)*/);
	...
	CPen bluePen;
	bluePen.CreatePen(PS_SOLID, 2, m_crButtonBorder);
	CPen* pOldPen = pBufferDC->SelectObject(&bluePen);

	pBufferDC->Rectangle(&rcClient);
	pBufferDC->FillSolidRect(rcClient, m_crButtonBackground);
...
LRESULT CSkinButton::OnSetCheck( WPARAM wParam, LPARAM lParam )
{
