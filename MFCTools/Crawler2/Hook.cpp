...
static _cc_rbtree_t gProcessModuleMap = {NULL};
...
static void _CC_CALL_ _PushMap(HICON hIcon, const tchar_t* szModule, DWORD dwBaseAddress, DWORD dwBaseSize) {
	...
	if (result == 1) {
		...
		_left->dwBaseSize = dwBaseSize;
		_left->hIcon = hIcon;
		_cc_rbtree_insert(&gProcessModuleMap, &_left->node, parent, node);
	}
}
...
{
	_cc_rbtree_destroy(&gProcessModuleMap, _FreeMap);
#if 1
...
tagProcessModule* _CC_CALL_ FindProcessModule(const tchar_t *szModuleName) {
	_cc_rbtree_iterator_t *node = _cc_rbtree_get(&gProcessModuleMap, (pvoid_t)szModuleName, _FindMap);
	if (node) {
...
_cc_rbtree_iterator_t* _CC_CALL_ FirstProcessModule() {
	return _cc_rbtree_first(&gProcessModuleMap);
}

_cc_rbtree_iterator_t* _CC_CALL_ LastProcessModule() {
	return _cc_rbtree_last(&gProcessModuleMap);
}
...
void _CC_CALL_ EnumProcess(bool_t (_CC_CALL_ *func)(LPPROCESSENTRY32, pvoid_t), pvoid_t args)
{
	...
	if(hSnap == INVALID_HANDLE_VALUE) {
		_cc_logger_error(_T("[-]CreateToolhelp32Snapshot with error %s\n"), _cc_last_error(_cc_last_errno()));
		return ;
	}
...
bool_t _CC_CALL_ EnableDebugPrivileges()
{
	...
	if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
		_cc_logger_error(_T("[-] OpenProcessToken error with %s"),_cc_last_error(_cc_last_errno()));
		return FALSE;
	}

	if(!LookupPrivilegeValue(NULL, SE_DEBUG_NAME, &Luid )) {
		_cc_logger_error(_T("[-] LookupPrivilegeValue error with %s"),_cc_last_error(_cc_last_errno()));
		CloseHandle(hToken);
		return FALSE;
	}
	...
	if(!AdjustTokenPrivileges(hToken, FALSE, &tkp, sizeof(tkp), NULL, NULL)) {
		_cc_logger_error(_T("[-] AdjustTokenPrivileges error with %s"),_cc_last_error(_cc_last_errno()));
		CloseHandle(hToken);
		return FALSE;
	}
	return TRUE;
